...
export class UsuarioService {
    ...
    static async exibirUsuario(id) {
        if (isNaN(id)) {
            throw new AppError('Id precisar ser um numero', 400)
        }
    ...
    static async registrarUsuario(nome, email, senha) {
        ...
        if (emailExiste) {
            throw new AppError('Já existe um usuario com este email', 409)
        }
    ...
    static async login(email, senha) {
        if (!email || !senha) {
            throw new AppError('E-mail e senha são obrigatórias!', 400)
        }
    ...
    static async update(id, nome, email, senha) {
        ...
        if (!nome && !email && !senha) {
            throw new AppError('que você está fazendo?', 400)
        }
        const usuarioExiste = await UsuarioRepository.buscarPorId(parseInt(id))
        if (usuarioExiste) {
        ...
        if (email) {
            ...
            if (emailExist) {
                throw new AppError('Este email já existe.', 400)
            }
        }

        nome = nome ?? usuarioExiste.nome
        email = email ?? usuarioExiste.email
        senha = senha ?? usuarioExiste.senha
